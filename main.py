...
def setup_screen():
    ...
    screen.title("The Youger Maker - Racing Turtles")
    ...
    turt.goto(-100, 205)
    turt.color("white")
    turt.write("Racing Turtles", font=("Courier New", 20, "bold"))
...
def race(user_guess: int):
    while(turtle1.xcor() <= FINISH_X  and turtle2.xcor() <= FINISH_X and turtle3.xcor() <= FINISH_X and turtle4.xcor() <= FINISH_X):
        ...
        if turtle1.xcor() > FINISH_X:
            winner = 1
        if turtle2.xcor() > FINISH_X:
            winner = 2
        if turtle3.xcor() > FINISH_X:
            winner = 3
        if turtle4.xcor() > FINISH_X:
            winner = 4
    ...
    if (user_guess == winner):
        ...
        screen.textinput("Game Over!", "You lose! Turtle " + str(winner) + "won the game")
